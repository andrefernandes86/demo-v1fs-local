...
docker run --rm \
    ...
    tmfs-cli-scanner scan "$LOCAL_PATH/test-eicar.txt"
...
docker run --rm \
    ...
    -e TM_TLS=false \
    -v "$LOCAL_PATH:$LOCAL_PATH:shared" \
    tmfs-cli-scanner scan-dir "$LOCAL_PATH/test-dir"

print_status "Testing Makefile with local path..."

# Test using Makefile
echo "🔍 Testing Makefile with local path..."
make -f Makefile.cli scan FILE="$LOCAL_PATH/test-eicar.txt" TM_ENDPOINT=my-release-visionone-filesecurity-scanner:50051 TM_TLS=false LOCAL_PATH="$LOCAL_PATH"
...
echo "  - Run: make -f Makefile.cli monitor LOCAL_PATH=$LOCAL_PATH TM_ENDPOINT=my-release-visionone-filesecurity-scanner:50051 TM_TLS=false"
echo "  - Run: make -f Makefile.cli scan FILE=/path/to/file LOCAL_PATH=$LOCAL_PATH TM_ENDPOINT=my-release-visionone-filesecurity-scanner:50051 TM_TLS=false"
echo "  - Run: docker run --rm -e TM_ENDPOINT=my-release-visionone-filesecurity-scanner:50051 -e TM_TLS=false -e LOCAL_PATH=$LOCAL_PATH -v $LOCAL_PATH:$LOCAL_PATH:shared tmfs-cli-scanner monitor" 